...
describe('sessionRepo', () => {
	describe('buildSessionQuery', () => {
		it('does not include person or su when not requested', async () => {
			//Arrange
			...
			expect(session.su).toBeUndefined();
		});
		it('includes person when requested', async () => {
			//Arrange
	...
	describe('findByUserKey', () => {
		...
		it('returns the session when it exists', async () => {
			const { sessionId, userkey } = await factories.session.createTestSession();
