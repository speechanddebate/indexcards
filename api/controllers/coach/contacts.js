...
export const updateContact = {
	...
	POST: async (req, res) => {
		...
		const firstStatusCheck = await checkContacts.POST(req, res);
		...
		await contact.save();
		const secondStatusCheck = await checkContacts.POST(req, res);
		...
		if (secondStatusCheck !== 'OK') {
			res.status(200).json({
				...
				error   : false,
				replace : [
					{ id : 'contact_errors', content: secondStatusCheck.join('<br />') },
				],
			});
		}
